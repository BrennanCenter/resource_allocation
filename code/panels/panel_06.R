...
wait <- readRDS("./temp/waits_2006.rds") %>% 
  filter(race == "Black")
...
match_data <- wait %>% 
  ...
  dplyr::select(birthyr, income, gender, college, married, married_noans, dem, rep, indep, over65, share_english_very_well, pop_den)
...
save(genout, file = "./temp/w06_genout.rdata")
...
saveRDS(matches, "./temp/balanced_06.rds")